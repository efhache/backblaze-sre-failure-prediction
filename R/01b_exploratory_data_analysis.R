...
# ==============================================================================
# 3. EDA 2: Failure Rate by Drive Model (Top 10 Most Common Models)
# ==============================================================================
...
), by = model][Total_Obs > 50000][order(-Total_Obs)][1:10]

p2 <- ggplot(model_stats, aes(x = reorder(model, Failure_Rate_pct), y = Failure_Rate_pct)) +
  geom_col(fill = "#3498db", width = 0.6) +
  coord_flip() +
  labs(
    title = "Hard Drive Failure Rate by Model (Top 10 Volume Models)",
    subtitle = "Percentage of daily records marked with acute hardware failure",
    x = "Drive Model",
    y = "Daily Failure Rate (%)"
  ) +
  scale_y_continuous(labels = function(x) paste0(x, "%"))

ggsave(file.path(PATH_FIGS, "fig2_failure_rate_by_model.png"), plot = p2, width = 9, height = 5, dpi = 300)
...
# ==============================================================================
# 5. EDA 4: S.M.A.R.T. Features Correlation Heatmap
# ==============================================================================
...
# Impute temporary missing values with 0 for correlation check
cor_data <- copy(dt[, ..smart_cols])
for (j in smart_cols) set(cor_data, which(is.na(cor_data[[j]])), j, 0)

cor_mat <- cor(log1p(pmax(0, as.matrix(cor_data))), method = "spearman")
cor_dt  <- as.data.table(melt(cor_mat))

p4 <- ggplot(cor_dt, aes(x = Var1, y = Var2, fill = value)) +
  ...
  labs(
    ...
    subtitle = "Identification of feature redundancy and collinearity among raw indicators",
    x = "", y = ""
  )

ggsave(file.path(PATH_FIGS, "fig4_correlation_heatmap.png"), plot = p4, width = 8, height = 6, dpi = 300)
rm(cor_data, cor_mat, cor_dt)
...
dt[, capacity_tb_cat := round(capacity_bytes / 1e12)]

cap_stats <- dt[, .(
  Total_Obs = .N,
  Failures = sum(failure),
  Failure_Rate_pct = (sum(failure) / .N) * 100
), by = capacity_tb_cat][Total_Obs > 10000][order(capacity_tb_cat)]

p6 <- ggplot(cap_stats, aes(x = factor(capacity_tb_cat), y = Failure_Rate_pct)) +
  geom_col(fill = "#2ecc71", width = 0.6) +
  geom_text(aes(label = sprintf("%.3f%%", Failure_Rate_pct)), vjust = -0.3, size = 3.5, fontface = "bold") +
  scale_y_continuous(expand = expansion(mult = c(0, 0.15)), labels = function(x) paste0(x, "%")) +
  labs(
    title = "Hard Drive Failure Rate by Capacity (TB)",
    subtitle = "Comparison of failure susceptibility across drive storage sizes (>10k daily records)",
    x = "Storage Capacity (TB)",
    y = "Daily Failure Rate (%)"
  )

ggsave(file.path(PATH_FIGS, "fig6_failure_rate_by_capacity.png"), plot = p6, width = 8, height = 5, dpi = 300)
rm(cap_stats)
