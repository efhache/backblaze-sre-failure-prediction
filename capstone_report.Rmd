...
While the broader literature explores failure horizons ranging from 14 to 30 days to facilitate large-scale logistical planning, this study specifically focuses on a 7-day prediction window ($H = 7$). In an SRE operational context, a 7-day window strikes an optimal balance: Defining a 7-to-30-day proactive window addresses critical infrastructure requirements: it provides distributed storage orchestrators (e.g., Google's Colossus) and SRE teams sufficient lead time to transparently drain data blocks from degrading drives to healthy nodes without impacting user-facing Service Level Objectives (SLOs) or incurring emergency maintenance toil (Pinheiro et al., 2007; Beyer et al., 2016).
The reason for choosing a 7-day window will be explained in the following paragraph.
...
While the theoretical literature considers extended prediction windows ranging from 7 to 30 days (Pinheiro et al., 2007; Beyer et al., 2016), this study deliberately restricts its empirical implementation to a 7-day prediction window ($H = 7$). This methodological trade-off is driven by two core operational and technical factors:

* **Signal-to-Noise Ratio and Signal Density:** Physical hard drive failure mechanisms—particularly those reflected in critical S.M.A.R.T. attributes such as pending sector counts (SMART 197) and reallocated sectors (SMART 5)—often exhibit highly localized, non-linear acceleration in the days immediately preceding complete breakdown. Extending the target window to 14 or 30 days introduces temporal noise and diffuses the predictive signal, leading to a higher rate of false positives. A 7-day lead time captures the sharpest period of degradation, significantly maximizing the class separability for tree-based models (Random Forest and XGBoost).
* **Computational Efficiency and Memory Constraints:** From an infrastructure perspective, computing rolling temporal deltas ($\Delta_7$) and lag features across multi-year telemetry datasets imposes severe RAM overhead during shift() and down-sampling operations in R. Restricting the target window to 7 days drastically reduces memory consumption, ensuring full pipeline execution within the constrained 6 GB RAM resource profile of the virtual machine while preserving spatial-temporal telemetry integrity. This point regarding the virtual machine’s limitations is explained below in the chapter ‘Design by Constraint: VM Computational Strategy’

From an SRE operational standpoint, a 7-day window remains fully actionable. It provides automated orchestrators (such as Google’s Colossus) and infrastructure teams sufficient lead time to transparently migrate data blocks and re-balance storage nodes without violating availability SLOs or latency budgets (Thurgood et al., 2018).
...
* **Extreme Class Imbalance:** Hard drive failures (`failure = 1`) represent a tiny fraction of total operational records—exactly $978$ failures out of $25,189,213$ daily logs ($\approx 0.0039\%$). When extended to the $14$-day prediction horizon ($Y_{it}$), positive failure instances increase to $5,247$ records ($0.0398\%$) within the filtered Top 5 fleet. This severe class imbalance dictates specialized resampling or cost-sensitive learning algorithms during downstream model formulation to avoid classifier bias toward the majority class. 
* **Fleet Homogeneity and Model Concentration:** As detailed in the model distribution table, the hard drive infrastructure is heavily concentrated around a few primary drive families. Toshiba models (`MG07ACA14TA` and `MG08ACA16TA`) together constitute over $26.77\%$ of the overall telemetry population, followed by high-capacity Seagate (`ST16000NM001G`), Western Digital (`WDC WUH721816ALE6L4`), and Seagate (`ST12000NM0008`) units. Together, these Top 5 models represent $52.29\%$ of the global dataset ($13,172,916$ daily logs). Filtering out fringe, low-count drive models beyond this Top 5 is thus an essential step to mitigate noise and prevent model overfitting to rare hardware architectures. Within this filtered Top 5 fleet (Table 4), Toshiba models (`MG07ACA14TA` and `MG08ACA16TA`) alone constitute $51.19\%$ of the operational subset ($26.25\%$ and $24.94\%$ respectively).
* **SMART Feature Heterogeneity:** Raw SMART attributes display distinct long-tailed distributions. Critical health metrics (such as SMART 5 for reallocated sectors or SMART 187 for uncorrectable errors) remain flat at zero for healthy drives but spike near failure, whereas operational metrics like SMART 9 (*Power-On Hours*) exhibit a continuous uniform distribution reflecting drive age.
...
Figure 4 presents the Spearman rank correlation matrix across raw SMART metrics. Two important structural collinearity patterns emerge:

1. **Near-Perfect Collinearity ($\rho = 1.00$) between SMART 197 (Pending Sectors) and SMART 198 (Uncorrectable Pending Sectors)**: In Backblaze architecture, these two features measure virtually identical sector-level degradation events. Including both raw metrics creates perfect feature redundancy; thus, feature engineering or model tree splitting can safely prioritize SMART 197 or its temporal delta ($\Delta_7$).
2. **Moderate Physical Coupling ($\rho = 0.33$) between SMART 5 and SMART 187**: Reflects the progressive physical transition where uncorrectable read errors lead to drive firmware reallocating physical sectors.
3. **Orthogonality of Temperature ($\rho \approx -0.01$ to $-0.02$)**: Confirms that thermal metrics operate independently of sector-level degradation variables.
...
### Predictive Window Optimization and Engineering Trade-Offs

While the initial exploratory data analysis examines a 14-day target horizon (`PREDICTION_WINDOW_DAYS <- 14`) to observe macro-level failure distributions across the Backblaze telemetry, the subsequent modeling pipeline explicitly refines this target to a strict 7-day projection window ($H = 7$). This transition reflects a practical engineering trade-off: empirical analysis during feature extraction demonstrated that computing 14-day rolling lag windows across millions of daily observations imposed prohibitive RAM overhead during memory-bound operations within the 6 GB virtual machine. By restricting the final predictive horizon to 7 days, the model effectively isolates the steepest degradation signals (such as sudden accelerations in `smart_197_delta7`) while optimizing memory consumption to ensure full execution within the constrained hardware environment. Consequently, the 7-day rolling deltas ($\Delta_7$) act as high-frequency predictors of short-term structural degradation, aligning directly with the operational requirements of proactive SRE maintenance.

\newpage

# Machine Learning and Predictive Modelling in an SRE Context

This chapter presents the modelling strategy used to forecast imminent hard drive failures within a 7‑day proactive maintenance window. The approach integrates temporal label construction, strict leakage‑free train/test separation, class‑imbalance mitigation, and reliability‑oriented evaluation metrics. The modelling pipeline is designed to align with SRE operational constraints, where false negatives carry significant SLA penalties and proactive replacement reduces toil and protects error budgets.

## Construction of the Predictive Failure Target (H = 7 Days) {#sec-methodology-pred-failure-target}
To train a proactive failure model, binary target labels must reflect future failure events within an $H = 7$-day projection window. For each drive $i$ observed at date $t$, the prospective failure target $Y_{it}$ is defined as:

$$Y_{it} = \max_{k \in [1, 7]} (\text{failure}_{i, t+k})$$

Where $\text{failure}_{i, t+k} \in \{0, 1\}$ represents the ground-truth terminal status of drive $i$ at time $t+k$. This formulation transforms a point-in-time anomaly detection task into a time-series binary classification problem, giving data center administrators a 7-day operational lead time for drive replacement.
...
### Engineering Trade-Offs: Horizon Selection ($H = 7$)

To balance early hazard detection with computational constraints ($6\text{ GB}$ RAM limit), the 7-day rolling window delta ($\Delta_7$) was engineered as the core dynamic predictor:

$$\Delta_7 SMART_{i,t} = SMART_{i,t} - SMART_{i,t-7}$$

Rapid positive accelerations in $\Delta_7 SMART_{197}$ or $\Delta_7 SMART_{187}$ provide actionable short-term failure signals, giving SRE teams a 7-day lead time to queue drive replacements.
...
Because XGBoost probabilities were trained with a cost-weighting parameter (scale_pos_weight = 2157.16), raw predictions are shifted toward higher probabilities. Raw scores were first recalibrated to reflect true physical posterior probabilities prior to threshold scanning.
...
* **Academic & Operational Alignment ($H = 7$ Days):** While initial exploratory data analysis evaluated a 14-day horizon, the operational modeling pipeline optimized the target window to 7 days ($H = 7$). In high-density cloud storage architectures, a 7-day lead time strikes an optimal balance: it maximizes the signal-to-noise ratio during acute degradation phases while providing automated orchestrators (e.g., Google Colossus, Ceph) and hardware teams sufficient time to gracefully drain data blocks without violating availability or latency Service Level Objectives (SLOs).
...
3. **Reproducible "Design by Constraint":** Demonstrating that enterprise-scale telemetry datasets (>13 million daily logs) can be successfully processed, engineered, and modeled within a restricted virtual machine environment (6 GB RAM, 4 CPUs) validates the feasibility of lightweight AIOps pipelines.
...
* **Binary Horizon vs. Continuous RUL:** Binary classification discretizes degradation into fixed time windows ($H = 7$), losing granular time-to-failure dynamics.
