...
\begin{titlepage}
    \begin{center}
        ...
        \vspace{0.5cm}
        \LARGE
        A Cost-Sensitive Machine Learning Framework Bridging Predictive Maintenance and Site Reliability Engineering
        
        \vfill
        
        \large
        A CAPSTONE PROJECT REPORT PRESENTED \\
        BY \\
        ...
        FOR THE \\
        \textbf{HARVARDX PROFESSIONAL CERTIFICATE} \\
        IN \textbf{DATA SCIENCE}
...
Beyond the predictive modeling objective itself, this project also serves as a practical proof-of-concept for the adoption of Site Reliability Engineering (SRE) principles within infrastructure operations. By demonstrating how large-scale telemetry can be transformed into actionable maintenance decisions, the study evaluates the technical feasibility, operational relevance, and potential business value of predictive maintenance as an enabler of proactive reliability management.

## II. Predictive Strategy & System Design {-}
To balance early warning capabilities with high signal confidence, the target failure horizon was determined to be **$H = 14$ days** based on operational considerations. This window provides automated distributed orchestrators (e.g., Ceph, Google Colossus) adequate lead time to gracefully drain data blocks off failing drives while maintaining an optimal signal-to-noise ratio. 
...
Beyond its academic objectives, the project functions as a feasibility study for predictive maintenance within an SRE transformation context. The intention is not merely to assess forecasting performance, but also to evaluate whether telemetry-driven risk prediction can realistically support operational decision-making, reduce reactive interventions, and contribute to the broader shift toward proactive reliability engineering.
...
To address this challenge, this project leverages real-world telemetry provided by Backblaze, containing daily operational snapshots and S.M.A.R.T. attributes collected from large-scale production storage environments. The objective is to build, evaluate, and interpret supervised machine learning models capable of predicting whether a hard drive will fail within the next 14 days.
...
* Comparative modeling evaluating a baseline Logistic Regression model and an advanced XGBoost classifier.
...
# Keep at most the top 10 models for visualization clarity
...
# Dynamic subtitle based on the number of detected models
...
# Dynamic adjustment of the image height when there are many models
...
if (!"capacity_tb" %in% names(bench_dt)) {
  if ("capacity_bytes" %in% names(bench_dt)) {
    ...
    bench_dt[, capacity_tb := 12] # Default average capacity value in TB
  }
}
...
### Operational Sensitivity Analysis across Infrastructure Scenarios {#sec-sensitivity}
...
**Justification of SRE Cost Weights ($C_{FN} = \$500$ vs $C_{FP} = \$15$):**
The operational cost parameters assigned to False Negatives ($C_{FN} = \$500$) and False Positives ($C_{FP} = \$15$) serve as normalized economic weights derived from site reliability engineering literature and error-budget accounting models. $C_{FN}$ encapsulates emergency engineering toil, network bandwidth saturation during ungraceful rebuilds, and potential Service Level Objective (SLO) violation penalties. Conversely, $C_{FP}$ represents unnecessary technician intervention and diagnostic testing overhead. These values reflect relative operational trade-offs rather than rigid accounting line-items; sensitivity analysis across Scenarios A and B demonstrates that the cost-minimizing threshold ($\tau^*$) remains robust across a 5x increase in penalty severity (from $\$100/\$15 \approx 6.7:1$ up to $\$500/\$15 \approx 33.3:1$). Furthermore, under extreme class imbalance ($\pi \approx 0.04\%$), this cost-sensitive threshold framework directly complements PR-AUC evaluation by translating precision metrics into actionable financial savings.
...
**Precision-Recall AUC ($0.0516$) in Extreme Imbalance Settings:**
While an absolute PR-AUC of $0.0516$ may appear low in standard classification contexts, it represents a **129-fold Precision Lift** over the baseline random classifier ($\pi \approx 0.0004$ daily failure prevalence). In extreme rare-event settings, traditional ROC-AUC can be overly optimistic due to the overwhelming number of True Negatives. The PR-AUC provides an unvarnished evaluation of predictive power. Operationally, a precision of $\approx 5\%$ at high recall ($\ge 70\%$) is highly effective for SRE teams: a False Positive merely triggers an automated, non-disruptive background task (drain mode / VM live migration), whereas a False Negative leads to catastrophic hardware failure, ungraceful RAID rebuilds, and human on-call toil.
As detailed in Section \ref{sec-sensitivity}, sensitivity analysis confirms that the optimal cost-minimizing threshold ($\tau^* \approx 0.012$) remains robust across a 5x increase in penalty severity ($6.7:1 \le C_{FN}/C_{FP} \le 33.3:1$).
...
As mentioned in the introduction, in addition to the predictive performance achieved in this study, the project demonstrates the practical feasibility of integrating machine learning into SRE-focused maintenance processes. The results suggest that predictive maintenance can move beyond being merely a theoretical concept of reliability to become a concrete operational capability, capable of supporting data-driven decision-making, maintenance planning and proactive infrastructure governance.
...
5. **Ensemble Learning Benchmark Expansion:**
   Future model extensions could include benchmarking additional ensemble approaches such as Random Forests, LightGBM, CatBoost, or Temporal Convolutional Networks in order to further assess the trade-off between predictive performance, explainability, and computational efficiency under constrained infrastructure environments.
...
\textbf{Program URL:} \textcolor{white}{https://harvardonline.harvard.edu/program/data-science} \\
\textbf{Harvard Portal:} \textcolor{white}{https://harvardonline.harvard.edu/} \\
...
\textbf{Date:} August-September 2026
\end{quote}
